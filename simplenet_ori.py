...
class SimpleNet(torch.nn.Module):
    ...
    def domainadaptembed(self, data):
        ...
        print('domain adaptation embed 시작 !!')
        if isinstance(data, torch.utils.data.DataLoader):
            features = []
            for image in data:
                if isinstance(image, dict):
    ...
    def domainadapt_embed(self, images, detach=True, provide_patch_shapes=False, evaluation=False):
        ...
        print('domain adaptation embed 중 !!')
        ...
        for i in range(1, len(features)):
            ...
            print('3', len(features[0]))
            _features = _features.permute(0, -3, -2, -1, 1, 2)
            print('4', len(features[0]))
            perm_base_shape = _features.shape
            _features = _features.reshape(-1, *_features.shape[-2:])
            print('5', len(features[0]))
            _features = F.interpolate(
                _features.unsqueeze(1),
                size=(ref_num_patches[0], ref_num_patches[1]),
                mode="bilinear",
        ...
        print('7', len(features[0]), len(features[1])) #here 6728
        # As different feature backbones & patching provide differently
        # sized features, these are brought into the correct form here.
        features = self.forward_modules["preprocessing"](features) # pooling each feature to same channel and stack together
    ...
    def _train_discriminator(self, input_data):
        ...
        dm_classifier = DomainClassifier().to("cuda")
        lam =  0.01
        momentum = 0.9 
        lr = 1e-3
        criterion = nn.CrossEntropyLoss()
        ...
        with tqdm.tqdm(total=self.gan_epochs) as pbar:
            for i_epoch in range(1, epochs+1):
                ...
                for src_data, tgt_data  in zip(src_train_loader, tgt_train_loader):
                    ...
                    print(tgt_data[0].shape)
                    print(tgt_data[1].shape)
                    ...
                    src_label_dm = torch.ones(tgt_data.shape[0]).to("cuda").long()
                    tgt_label_dm = torch.zeros(tgt_data.shape[0]).to("cuda").long()
                    ...
                    src_feature, _ = self.domainadapt_embed(src_data.to("cuda"), evaluation=False)
                    tgt_feature, _ = self.domainadapt_embed(tgt_data.to("cuda"), evaluation=False)
